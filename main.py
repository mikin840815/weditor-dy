...
class QuitHandler(page.BaseHandler):
    def get(self):
        stop_server()
        self.write({"success": True, "description": "Successfully quited"})


class CropHandler(page.BaseHandler):
    def get(self):
        """ used for crop image """
        pass
...
def make_app(settings={}):
    application = tornado.web.Application(
        [
            ...
            (r"/api/v1/devices/([^/]+)/widget", page.DeviceWidgetListHandler),
            (r"/api/v1/widgets", page.DeviceWidgetListHandler),  # add widget
            (r"/api/v1/widgets/([^/]+)", page.DeviceWidgetListHandler),
            # v2
            (r"/api/v2/devices/([^/]+)/hierarchy", page.DeviceHierarchyHandlerV2),
            # widgets
            (r"/widgets/([^/]+)", page.WidgetPreviewHandler),
